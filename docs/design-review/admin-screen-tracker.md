...
| `trip-detail.html` | #1671, #1672 | ⚠️ | All three shapes verified: completed (8-step timeline, fare breakdown arithmetic checks out, rating + tags, route section), expired (expiry reason in summary and timeline, estimate shown, fare/rating/route hidden), active (estimate + intervention panel). #1808 cancel and #1809 reassign are now **built as a proposal** (both stories still unwritten) — verified: reason required, note mandatory on "Other", cancel sets a distinct Cancelled status and hides the intervention block, reassign swaps the driver and appends reassigned-by-admin → accepted to the timeline, both persist to the trip list and reach the audit log. **Route map unverified** — environment. |
