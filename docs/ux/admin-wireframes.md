...
| 3c | **Reinstatement now records a reason, for drivers and riders.** Neither #1743 (driver) nor #1741 (rider) asks for one, but reinstating is as consequential as suspending — #1742 and #1740 both require a reason — and an unexplained status flip leaves no audit trail. Both mockup screens require a reason from a list, make a note mandatory on "Other", show the recorded reason with actor and timestamp on the profile, and write a `reinstate` entry to the audit log, which #1816 already lists as auditable. Suspension now shows its actor and timestamp too, so the two blocks match. Riders carry no decision-history timeline, so a `suspendedBy` field was added to the rider record. #1741 and #1743 both need the reason added. | Open — stories need updating |
