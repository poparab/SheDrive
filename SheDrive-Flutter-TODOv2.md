...
- [ ] On success, persist session + route based on `identityVerified`.

### 2.5 Rider eKYC
- [ ] Replace `onTap: () => setState(() => _idUploaded = true)` with `image_picker` capture → show real thumbnail → allow retake.
- [ ] National ID: **front + back** as two captures (subtitle says "Front and back" but only one capture happens today).
- [ ] Selfie: front camera + face-oval guide overlay.
- [ ] On submit: show a 2 s "verifying…" spinner, then succeed by default; demo helper can flip to "rejected" for the negative path.
- [ ] Persist `identityVerified=true` on approval.
- [ ] Localize all strings.
- [ ] "Why we ask" tap-target with a short privacy explanation.
